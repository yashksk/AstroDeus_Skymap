...
class CPReduce {
    ...
    */
	...
	static convertGeodedicLatLonToECEFXYZ(lat, lon, height) {
		...
		const C =
			1 / Math.sqrt(Math.cos(lat) * Math.cos(lat) + (1.0 - f) * (1.0 - f) * (Math.sin(lat) * Math.sin(lat)));
	...
	static rotvsop2J2000(x) {
        ...
        */
		let t = new Array();
		t[0] = x[0] + x[1] * 0.00000044036 + x[2] * -0.000000190919;
	...
	static nutation2000BTruncated(T) {
		...
		const D = AS2R * (1072260.70369 + 1602961601.209 * T - 6.3706 * T2 + 0.006593 * T3 - 0.00003169 * T4);
	...
	static earthRotationAngle(jd_ut1) {
		...
		let era = (Math.PI * 2 * (0.779057273264 + 0.00273781191135448 * t + frac)) % (Math.PI * 2);
