...
sub processVersion(){
	...
	$version=$version."_".$accversion."_velocities";
...
sub generatePublicFunctions(){
	...
	foreach my $body (@bodies){
		...
		foreach my $var (@vars){
			print $out "      temp[$i]=$body"."_$var"."(t) / 365250.0;\r\n";
			$i++;
		}
	...
	while(my $l=<$f>){
		if(substr($l,0,7) eq ' VSOP87'){
			...
			if($a>=$accuracy || $accuracy==0){
				...
				print $out "      $variable+=";
				if($exponent!=0){
					print $out "Pow(t,exp-1)*exp*$a*Cos($b + $c*t)";
				}
				print $out "-Pow(t,exp)*$a*$c*Sin($b + $c*t);\r\n";
				$hasTerms=1;
			} else {
				$skippedTerms++;
			}
