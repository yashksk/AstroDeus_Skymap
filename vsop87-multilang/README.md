...
VSOP87 authors, a validation program is also included for each language.  They are ready for use. See the "Languages" directory for the status of each language.
...
, for more languages, and have a more permissive license (public domain).  It will provide tests against the original VSOP87 test data for each programming language to validate correctness, as well as examples to show common usage.
...
developed before and after VSOP87, it remains one of the most popular methods among amateurs.  It provides better accuracy than most amateurs require (.1 arcseconds)
...
fast enough, from there you should experiment with truncated versions.  The VSOP87A versions are the only versions which include both the Earth and Moon.  VSOP87A doesn't include the moon directly, but does include the Earth and the Earth-Moon Barycenter, and all provided code for the VSOP87A versions include a function to compute the Moon's position from the Earth and EMB.  Using the versions that provide the velocities is necessary if you want to account for relativistic effects do to the motion of the observer.
...
-------|-------|-----|-----|---|----|-------|------|------|-------|---|-----------
VSOP87|Yes|Yes|No|Yes|Yes|Yes|Yes|Yes|Yes|No|Keplerian Orbital Elements
...
Truncation Level|Total Terms|Skipped Terms|Percent Skipped
----------------|-----------|-------------|---------------
full 	 |269949|0     |0 %
...
are available for the Earth-Moon Barycenter (EMB), the error for the Moon will be a linear function of the EMB error.
